...
export const ChatInterface: React.FC = () => {
  ...
  const { addToast, setShowApiKeyModal } = useUiStore();
  ...
  const executeSingleGeneration = async (text: string, attachments: Attachment[]) => {
    ...
    try {
      ...
      if (settings.streamResponse) {
          ...
          if (isThinking) {
              thinkingDuration = (Date.now() - startTime) / 1000;
              const lastMsg = useAppStore.getState().messages.slice(-1)[0];
              if (lastMsg?.parts) {
                updateLastMessage(lastMsg.parts, false, thinkingDuration);
              }
          }
      } else {
